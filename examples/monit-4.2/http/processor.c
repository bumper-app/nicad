 ...
 *
 *  @version \$Id: processor.c,v 1.39 2004/02/18 22:31:42 chopp Exp $
 *
 *  @file
 */
 ...
 * specified redirect Location URL. The url must be absolute.
 * @param res HttpResponse object
 * @param location An absolute url to redirect to
 */
...
void out_print(HttpResponse res, const char *m, ...) {
  ...
  if(m) {
    ...
    if(have <= need) {
      ...
      res->outputbuffer=
	  xresize(res->outputbuffer,(res->bufsize + need + RES_STRLEN));
  ...
  while(1) {
    ...
    if( NULL != (value= strchr(line, ':'))) {
      ...
      *value++= '\0';
      trim(line);
      trim(value);
      /* ATTENTION: Please double check this calculations: */
...
static void reset_response(HttpResponse res) {
  ...
  if(res->headers)destroy_entry(res->headers);
  memset(res->outputbuffer, 0, res->bufsize);
  res->bufused= 0;
...
static void destroy_HttpRequest(HttpRequest req) {
  ...
  if(req) {
    ...
    if(req->headers)destroy_entry(req->headers);
    if(req->params)destroy_entry(req->params);
    FREE(req);
...
/**
 * Free a (linked list of) http entry ojbect(s). Both HttpHeader and
...
static int basic_authenticate(HttpRequest req) {
  ...
  if(credentials) {
    ...
    if(decode_base64(cr, strchr(credentials, ' '))) {
      ...
      /* Extract the username the user */
      strncpy(uname, cr, unamelen);
      uname[unamelen]='\0';

      /* Check if user 'uname' does exist */
	...
	log("Warning: Client '%s' supplied unknown user '%s'"
        ...
        passwd=strchr(cr,':')+1;
        /* Check if 'uname' has supplied the right passwd */
